...
- [x] Review `Deployment/DEPLOY_FILEBEAT*.md`, `DATASETS.md`, `ALERTS.md`,
      ...
      **Done** —
      - `DEPLOY_NG_FEED.md`: rewrote the compose service snippet
        (`linuxserver/syslog-ng` → `balabit/syslog-ng:latest`, ports
        `514:5514/udp` + `601:6601/tcp`, config mount, `depends_on: broker`),
        replaced the config examples with the canonical
        `infrastructure/syslog-ng/syslog-ng.conf` (broker:29092, `$(format-json)`
        payloads), fixed the advanced-routing topics to the
        `logs-prod-nonpci-<source>` convention and the client ports.
      - `DEPLOY_FILEBEAT.md`: added a banner (current deployment = Compose
        Filebeat → Kafka, k8s node logs = FluentBit); removed the stale
        `elastic:changeme` / `curl -u elastic:password` ES auth (security is
        disabled); fixed the OS-collector `output.kafka` (was
        `connect:8083`/`filebeat-topic` → `broker:29092`/
        `logs-prod-nonpci-filebeat`).
      - `DEPLOY_FILEBEAT_DOCKER.md`: aligned input config + paths with the
        canonical `data/filebeat/config/filebeat.yml`; fixed volume path and
        `ping broker`.
      - `FLUENTBIT_SELECTIVE_INGEST.md`: CRI parser + `/var/log/containers`
        paths (not `docker` parser `/var/lib/docker/containers`),
        `brokers broker:29092`, topic `logs-prod-nonpci-fluentbit`,
        manifest path `k8s/3.01.fluent-bit-config.yaml`.
      - `DEPLOY_SYSLOG.md`: corrected the deprecated k8s reference section's
        `bootstrap_servers("connect:8083")` → `broker:29092`.
      - `DATASETS.md` + `ALERTS.md`: reviewed — already aligned (8 buckets,
        topic/path/SLM conventions; generic Jira-alerting guide, no stale
        deployment refs).
