...
route to services in the `elastic` namespace (Kibana, Elasticsearch).
...
| File | Contents |
|------|----------|
| `k8s/4.01.traefik-crds.yaml` | Traefik v3 CRDs (`IngressRoute`, `Middleware`, `MiddlewareTCP`, ...) |
| `k8s/4.02.traefik-rbac.yaml` | `ingress-traefik1` namespace + ServiceAccount + ClusterRole/Binding |
| `k8s/4.03.traefik-deploy.yaml` | Traefik Deployment + ClusterIP Service (`traefik1`) |
| `k8s/4.04.traefik-ingressroutes.yaml` | Middlewares + IngressRoutes (Kibana + ES routes) |
...
- Cross-namespace routing requires
  ...
  `k8s/4.03.traefik-deploy.yaml`).
- Traefik RBAC (`k8s/4.02.traefik-rbac.yaml`) must include `endpointslices`,
...
- The Elasticsearch REST API is exposed unauthenticated on purpose for this
  homelab deployment (security is disabled in `k8s/1.02.elasticsearch.yaml`).
